...
const revisionBoundLearningTargetSchema = z.object({
	id,
	revision,
	type: z.enum(["prompt", "source", "material"]),
});
const readingLearningTargetSchema = z.object({
	id,
	revision,
	segmentId: id,
	type: z.enum(["source-segment", "material-segment"]),
});
const stableLearningTargetSchema = z.object({
	id,
	type: z.enum(["collection", "concept"]),
});
export const learningTargetSchema = z.union([
	revisionBoundLearningTargetSchema,
	readingLearningTargetSchema,
	stableLearningTargetSchema,
]);
const readingSegmentSchema = z.object({
	...
	target: z.object({
		id,
		revision,
		type: z.enum(["source", "material"]),
	}),
});
const learningObservationSchema = z.object({
	activityKind: z.enum(["recall", "practice", "read", "lesson"]),
...
export const corpusDocumentSchema = z.object({
	...
	learningObservations: z
		.array(learningObservationSchema)
		.optional()
		.default([]),
	...
	readingSegments: z.array(readingSegmentSchema).optional().default([]),
...
export type LearningTarget = z.output<typeof learningTargetSchema>;
export type ReadingSegment = z.output<typeof readingSegmentSchema>;
export type LearningObservation = z.output<typeof learningObservationSchema>;
