...
type CompiledLineageApi = {
	...
	learningTargetKind: AgdaConstructor;
	segmentOwnerKind: AgdaConstructor;
	learningActivityKind: AgdaConstructor;
	learningObservationKind: AgdaConstructor;
	...
	learningTargetReference: AgdaConstructor;
	readingSegment: AgdaConstructor;
	learningObservation: AgdaConstructor;
...
function toAgdaCorpus(api: CompiledLineageApi, document: CorpusDocument) {
	...
	const learningTarget = (
		value: CorpusDocument["learningObservations"][number]["target"],
	) =>
		apply(
			api.learningTargetReference,
			api.learningTargetKind(value.type),
			value.id,
			maybe("revision" in value ? natural(value.revision) : undefined),
			maybe("segmentId" in value ? value.segmentId : undefined),
		);
	const readingSegments = document.readingSegments.map((value) =>
		apply(
			api.readingSegment,
			value.id,
			api.segmentOwnerKind(value.target.type),
			value.target.id,
			natural(value.target.revision),
			natural(value.ordinal),
			value.content,
		),
	);
	const learningObservations = document.learningObservations.map((value) =>
		apply(
			api.learningObservation,
			value.id,
			learningTarget(value.target),
			api.learningActivityKind(value.activityKind),
			api.learningObservationKind(value.observationKind),
			value.observedAt,
			maybe(
			...
			maybe(value.assessment),
			maybe(value.response),
			value.provenance,
		),
	);
